...
png("PGC_EWAS_inverseNorm_QQplot_civilian_forPublication.png", width=1600, height=800, units="px", bg="white")
par(mar=c(10,10,4,2), mgp=c(6,2,0))
ggd.qqplot(pvalue, main = paste(""))
dev.off()
...
png("PGC_EWAS_manhattan_civilian_forPublication.png",height=600, width=1200, units="px")
par(mar=c(5.1,6.1, 4.1, 2.1))
...
postscript("PGC_EWAS_manhattan_civilian.eps", horizontal=FALSE, width = 6, height = 3, 
...
png("PGC_EWAS_manhattan_civilian_BW.png", width = 600, height = 300, units="px")
...
for(ii in 1:nrow(results)){
  ...
  summary<-summary[-1]
  cochrane<-cochrane[-1,]
  postscript(paste("PGC_EWAS_civilian_", results[ii, "CpG"], ".eps", sep=""), horizontal=FALSE, width = 3, height = 3, 
             onefile = FALSE, paper = "special", colormodel = "cmyk")
